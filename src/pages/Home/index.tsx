...

            {
                loading ? (
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => 
                            <ChatsList data={item} deleteChat={()=>deleteChat(item.owner,item._id)} userStatus={user}/>
                        }
                    />
                )
            }
            <Modal visible={modalVisibility} animationType='fade' transparent={true}>
                <ModalNewChat setVisibity={() => setModalVisibility(false)} 
                    setUpdateChats={()=>setUpdateChats(!updateChats)}
                />
            </Modal>
            <FabButton setVisibity={() => setModalVisibility(true)} userStatus={user} />
        </SafeAreaView>
    )
}